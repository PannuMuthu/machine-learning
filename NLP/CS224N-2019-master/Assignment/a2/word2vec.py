...
def naiveSoftmaxLossAndGradient(
    ...
    gradCenterVec   = outsideVectors.T.dot(d_value) # shape d x 1  Written Assignment part b
    gradOutsideVecs = d_value[:, np.newaxis].dot( np.array([centerWordVec]) ) # (N, 1) dot (1, d) -> (N, d) written assignment part c
...
def negSamplingLossAndGradient(
    ...
    value_outside  = o_vector.dot(centerWordVec) # U_o^T*V_c
    value_negative = neg_vector.dot(centerWordVec) # U_k^T*V_c

    p_outside  = sigmoid(value_outside)
    p_negative = sigmoid(- value_negative)

    loss = - (np.log(p_outside) + np.sum(np.log(p_negative)) )
    gradCenterVec = (p_outside - 1) * o_vector + np.sum((1 - p_negative)[:, np.newaxis] * neg_vector, axis = 0) # part e 
...
def skipgram(currentCenterWord, windowSize, outsideWords, word2Ind,
    ...
    ### YOUR CODE HERE
    # Used similar implementation as https://www.geeksforgeeks.org/implement-your-own-word2vecskip-gram-model-in-python/
